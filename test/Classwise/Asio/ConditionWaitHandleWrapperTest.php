<?hh // strict
namespace HHRx\Test\Classwise\Asio;
use HHRx\Test\CustomException;
use HHRx\Asio\ConditionWaitHandleWrapper;
use PHPUnit\Framework\TestCase;
class ConditionWaitHandleWrapperTest extends TestCase {
	private function _void_to_mixed(Awaitable<void> $incoming): Awaitable<mixed> {
		return $incoming;
	}
	private function make_empty_wrapper<T>(): ConditionWaitHandleWrapper<T> {
		return new ConditionWaitHandleWrapper(\HH\Asio\later()->getWaitHandle());
	}
	...
	public function test_reset_simple(): void {
		...
		$wait_handle = new ConditionWaitHandleWrapper($awaitable->getWaitHandle());
